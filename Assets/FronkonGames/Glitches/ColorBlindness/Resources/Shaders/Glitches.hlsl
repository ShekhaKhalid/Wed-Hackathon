...
inline float Remap(float target, float oldMin, float oldMax, float newMin, float newMax)
{
  return(target - oldMin) / (oldMax - oldMin) * (newMax - newMin) + newMin;
}

inline float2 Remap(float2 target, float oldMin, float oldMax, float newMin, float newMax)
{
...
{
  const half separatorWidth = 2.0 * _MainTex_TexelSize.x;
